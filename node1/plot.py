...
traffic_nobypass=pd.read_csv("trafficResultNobypass.csv", sep=' ')
time_nobypass=pd.read_csv("timeResultNobypass.csv", sep=' ')
...
blocking_P_nobypass=np.zeros(6000)
TrafficNum=0
TrafficSuccess=0
C_TrafficSuccess=0
N_TrafficSuccess=0
for i in range(6000):
    if(traffic_result['SuccessFlag'][i]==0):
        ...
        N_TrafficSuccess+=traffic_nobypass['SuccessFlag'][i]
    ...
    blocking_P_nobypass[i]=1-(N_TrafficSuccess/TrafficNum)
'''
...
plt.plot(range(6000),blocking_P_nobypass, label="No Bypass ROADM")
...
a.plot(time_result['time'],time_nobypass['trafficSuccessNum'], label="No Bypass")
...
device_N=time_nobypass['WSS1']+time_nobypass['WSS2']+time_nobypass['WSSadd']+time_nobypass['WSSdrop']+time_nobypass['split']
a=plt.subplot(212)
a.plot(time_result['time'],device, label="WDM/SDM")
a.plot(time_result['time'],device_N, label="No Bypass")
