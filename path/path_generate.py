...
def pathCost_Occupy(path):
    path_len=len(path)
    for i in range(path_len):
        InNode[path[i][0]][path[i][1]][path[i][2]]=1
        OutNode[path[i][0]][path[i][1]][path[i][2]]=1
        
        if(i>0 and path[i][2]!=bypass_core[path[i][0]][0] and path[i][2]!=bypass_core[path[i][0]][1]):
            linkMap[path[i-1][0]*21+path[i-1][1]*7+path[i-1][2]][path[i][0]*3+path[i][1]]=1
...
TrafficDF=pd.DataFrame(Trafficdf, columns=['SD', 'Bandwidth'])   
TrafficDF=TrafficDF.sort_values(by=['Bandwidth'],ascending=[False]) 
